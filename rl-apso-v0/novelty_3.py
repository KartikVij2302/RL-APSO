...
class PPOAgent:
    ...
    def update(self):
        ...
        # FIX: Optimization to avoid slow list-to-tensor conversion
...
class NovelPPO_PSO:
    ...
    def scale_action(self, action):
        """ Warm Start: Center around Golden Parameters """
...
def run_comparison():
    ...
    # Variables to store data for the LAST run (for detailed plotting)
    last_std_hist = []
    last_nov_hist = []
    last_rewards = []
    last_params = []
    
    print(f"--- Running Particle Sweep Experiment: {particle_sweep} ---")
    ...
    for n_particles in particle_sweep:
        ...
        # 1. Standard PSO
        start = time.time()
        std_pso = StandardPSO(rastrigin, dim=DIM, num_particles=n_particles, max_iter=ITERATIONS)
        ...
        # 2. Novel PPO PSO
        start = time.time()
        nov_pso = NovelPPO_PSO(rastrigin, dim=DIM, num_particles=n_particles, max_iter=ITERATIONS)
        ...
        # Save details if this is the last run in the sweep
    ...
    plt.tight_layout()
    plt.savefig('pso_sweep_comparison.png')
    print("\nResults Summary:")
    ...
    print("\nPlot saved as 'pso_sweep_comparison.png'")

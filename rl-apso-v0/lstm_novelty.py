...
class ObstacleEnvironment:
    def __init__(self):
        ...
        self.start_area = np.array([5.0, 5.0])
...
# ==========================================
class StandardPSO:
    def __init__(self, env, num_particles=30, max_iter=200):
        self.env = env
        self.dim = 2 # 2D Space
...
class LSTMActorCritic(nn.Module):
    ...
    def forward(self, state):
        ...
        lstm_out, (h_n, c_n) = self.lstm(state)
        
        # We take the last hidden state from the sequence
...
class PPOAgent:
    def __init__(self, state_dim, action_dim, lr=0.0003):
        ...
        self.policy_old = LSTMActorCritic(state_dim, action_dim)
    ...
    def select_action(self, state_seq):
        # State sequence shape: (Seq_Len, Features) -> Add batch dim (1, Seq, Feat)
        state = torch.FloatTensor(state_seq).unsqueeze(0)
        with torch.no_grad():
            action, logprob = self.policy_old.act(state)
        return action.cpu().numpy().flatten(), logprob

    def store(self, s, a, lp, r, d):
        # Store the raw sequence (numpy array)
...
# ==========================================
# 4. Novel PPO-PSO (Navigation + LSTM)
# ==========================================
class NovelPPO_PSO:
    def __init__(self, env, num_particles=30, max_iter=200):
        self.env = env
    ...
    def get_state_seq(self, i):
        ...
        snapshot = np.array([progress, dist_to_goal, diversity])
        
        # Update Normalizer
        self.normalizer.observe(snapshot)
        norm_snapshot = self.normalizer.normalize(snapshot)
        
        # Add to history buffer
        self.state_buffer.append(norm_snapshot)
        
        # Pad with zeros if buffer not full (start of episode)
        while len(self.state_buffer) < self.seq_len:
            self.state_buffer.appendleft(np.zeros(3))
    ...
    def run(self):
        ...
        state_seq = self.get_state_seq(0)
        
        for i in tqdm(range(self.max_iter), desc="Novel LSTM-PPO-PSO"):
            
            action, logprob = self.ppo_agent.select_action(state_seq)
            ...
            self.ppo_agent.store(state_seq, action, logprob, reward, i==self.max_iter-1)
            if i % self.update_timestep == 0: self.ppo_agent.update()
            
            self.prev_gbest = self.gbest_val
            state_seq = self.get_state_seq(i) # Get next sequence
            self.history_traj.append(self.X.copy())
...
def run_comparison():
    ...
    # Run Standard
    std_pso = StandardPSO(env)
    std_best, std_traj = std_pso.run()
    
    # Run Novel
    nov_pso = NovelPPO_PSO(env)
    nov_best, nov_traj = nov_pso.run()
    
    print(f"\nFinal Cost (Standard): {std_best:.2f}")
    print(f"Final Cost (Novel LSTM-PPO): {nov_best:.2f}")

    # Plotting
    fig, axes = plt.subplots(1, 2, figsize=(16, 8))
    
    titles = ["Standard PSO", "Novel LSTM-PPO-PSO"]
    ...
    plt.tight_layout()
    plt.savefig('swarm_obstacle_avoidance_2.png')
    print("Saved simulation plot to 'swarm_obstacle_avoidance_2.png'")
